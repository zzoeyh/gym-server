...
(
    ...
    groupId   int          not null comment '器材组编号',
    createdAt datetime     null,
    updatedAt datetime     null,
    constraint race_equipment_equipment_null_fk
...
create index race_equipment_group_id_index
    on race_equipment (groupId);

create table umpire
(
    id          int auto_increment comment '裁判id'
        primary key,
...
(
    ...
    updatedAt datetime     null,
    damage    int          null comment '损坏数量',
    borrow    int          null comment '租借数量',
    createId  varchar(255) null,
    status    int          null comment '记录状态',
    constraint user_equipment_equipment_null_fk
        foreign key (eid) references equipment (id),
    constraint user_equipment_user_id_fk
        foreign key (createId) references user (id)
)
    comment '用户租用器材表';
...
(
    ...
    umpireId    int          null comment '裁判id',
    status      int          null comment '状态',
    createdAt   datetime     null comment '创建时间',
    updatedAt   datetime     null comment '更新时间',
    constraint race_race_equipment_null_fk
        foreign key (reid) references race_equipment (groupId),
    constraint race_umpire_null_fk
        foreign key (umpireId) references umpire (id),
    constraint race_user_null_fk
        foreign key (createId) references user (id),
    constraint race_venue_id_fk
